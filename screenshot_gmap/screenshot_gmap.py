...
source_dir = '/mnt/git/nb/data/nbdata/SD_sold_fourview_01082022/gmap'

jsonlist = glob(os.path.join(source_dir, "split_json",'*.json'))
txt_download_path = '/mnt/git/nb/data/nbdata/SD_sold_fourview_01082022/gmap/download_record.txt'
txt_json_path = '/mnt/git/nb/data/nbdata/SD_sold_fourview_01082022/gmap/update_json_record.txt'
...
for i in range(len(jsonlist)):
    ...
    with open(txt_json_path, "a") as txt_file:
        txt_file.write(str(i) + '_update.json lenth: ' + str(len(doc_detail_screenshot)) + "\n")

    # write updated json
    json_update_file = os.path.join(source_dir, "update_json", str(i) + '_update.json')
    with open(json_update_file, 'w') as juf:
        json.dump({'document_detail': doc_detail_screenshot
                   }, juf)
    juf.close()